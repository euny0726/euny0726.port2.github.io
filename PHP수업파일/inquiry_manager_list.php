...
<?php
	...
	$sql = "select * from inquiry order by num desc";	// select 명령어 : 조회 / inquiry 테이블 num 필드 기준으로 order by : 숫자num 내림차순desc 정렬 (최신글 먼저 조회목적) cf.오름차순 : asc
	...
	// 전체 페이지 수($total_page) 계산 (페이지 넘버 구하는 if문)
   ...
   for ($i=$start; $i<$start+$scale && $i < $total_record; $i++)//$i 변수 선언	// && 는 and 조건, || 는 or 조건
   {
      mysqli_data_seek($result, $i);
      // 가져올 레코드로 위치(포인터(선택)) 이동
...
?>
		<li>
			<span class="col1"><?=$number?></span> <!--73행의 $number 97행에 $number--;로 -1씩 줄어듦.  -->
